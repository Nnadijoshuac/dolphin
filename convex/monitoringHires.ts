...
export const hireMonitoringAgent = mutation({
  args: {
    tokenId: v.string(),
  ...
  handler: async (ctx, { tokenId, walletAddress, priceModel }) => {
    if (!isAddress(walletAddress)) {
      throw new Error(`hireMonitoringAgent: "${walletAddress}" is not a valid EVM address.`);
    }
    ...
    if (priceModel === null) {
      throw new Error(
        "Cannot hire yet: this agent's priceModel has not resolved to a live value. " +
          "Wait for priceModel.status to be \"live\" or \"stale\" before calling hireMonitoringAgent - " +
    ...
    if (!isFreePriceModel(priceModel)) {
      ...
      throw new Error(
        `hireMonitoringAgent: agent ${tokenId}'s priceModel requires payment ` +
    ...
    return ctx.db.insert("monitoringHires", {
...
export const getHiredMonitoringAgentsForWallet = query({
  args: {
    walletAddress: v.string(),
  },
