...
export function useJobDelivery(job: AgentJobRow | null | undefined): JobDelivery {
   ...
   * restarting the clock, which would let someone refresh their way out of the
   * "taking longer than expected" state without anything having changed.
   *
   * The "now" side comes from the query's own `dataUpdatedAt` rather than a
   * `Date.now()` read during render (impure, and rejected by the React Compiler
   * lint rules) or a separate ticker. It advances on every successful poll, so
   * it is exactly as fresh as the reading it is describing - and it needs no
   * second timer to stay in step.
   */
  const startedAtMs = job ? Date.parse(job.verifiedAt) : Number.NaN;
  const elapsedMs =
    query.dataUpdatedAt === 0 || Number.isNaN(startedAtMs)
      ? 0
