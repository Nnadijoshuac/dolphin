...
function liveIndexedMetric<T>(
  ...
  return { status: "live", value, asOf, source: AGENT_DATA_SOURCES.scan, methodology };
}
...
function unavailableIndexedMetric<T>(
  ...
  return { status: "unavailable", value: null, asOf, source: AGENT_DATA_SOURCES.scan, reason };
}
...
function decodeIndexedAgent(payload: unknown, fallback: Agent): Agent {
  ...
  const isActiveVal = readBoolean(payload, "is_active");
  ...
  return {
    ...
    isActive:
      isActiveVal === null
        ...
        : liveIndexedMetric(isActiveVal, indexedAt),
    reputationScore,
    feedbackCount,
