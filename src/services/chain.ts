...
import type { LiveMetric, RegistryVerification } from "@/types/agent";
...
export async function verifyAgentRegistration(
  ...
  const owner =
    ownerResult.status === "fulfilled"
      ? liveRegistryMetric(ownerResult.value, checkedAt)
  ...
  if (!includeTokenUri) {
    tokenUri = unavailableRegistryMetric(
      "The token URI check was not requested.",
      checkedAt,
    );
  } else if (
    ...
    tokenUri = unavailableRegistryMetric(
      "The registry did not return a token URI.",
      checkedAt,
    );
  }
  ...
  if (!includeAgentWallet) {
    agentWallet = unavailableRegistryMetric(
      "The agent wallet check was not requested.",
      checkedAt,
    );
  } else if (
    agentWalletResult.status === "fulfilled" &&
    agentWalletResult.value !== null &&
    agentWalletResult.value.toLowerCase() !== zeroAddress
  ) {
    agentWallet = liveRegistryMetric(agentWalletResult.value, checkedAt);
  } else {
    agentWallet = unavailableRegistryMetric(
