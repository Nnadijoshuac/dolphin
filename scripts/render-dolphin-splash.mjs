...
try {
  ...
  if (!preview) {
    ...
    // Keep the rendered sRGB transfer and explicitly convert with the BT.709
    // matrix. Check actual browser playback, not only ffmpeg-decoded PNGs:
    // Chrome's limited-range 8-bit conversion differs from #F6F4EE by just +1
    // green; compensating RGB for ffmpeg introduces a visible browser border.
    execFileSync('ffmpeg', ['-hide_banner', '-loglevel', 'warning', '-y', '-framerate', `${fps}`,
      '-i', join(frameDir, '%03d.png'), '-vf', 'scale=out_color_matrix=bt709:out_range=tv',
      '-c:v', 'libx264', '-preset', 'slow', '-crf', '18',
      '-profile:v', 'baseline', '-level', '3.0', '-pix_fmt', 'yuv420p', '-an',
      '-movflags', '+faststart', '-colorspace', 'bt709', '-color_range', 'tv',
      '-color_primaries', 'bt709', '-color_trc', 'iec61966-2-1', video]);
