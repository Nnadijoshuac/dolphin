...
import type { ReviewOutcome } from "@/convex/api";

/*
 * ===========================================================================
 * MIRRORED BY HAND from src/services/reputation-registry.ts in the mobile app,
 * which is itself mirrored by convex/lib/reputationRegistry.ts. (2026-09-08)
 * ===========================================================================
 * Three copies of one ABI, and that is deliberate rather than lazy: the mobile
 * app cannot import from web/, the website cannot import from the repo root
 * without breaking its clean-install guarantee (see the note at the top of
 * convex/api.ts), and Convex cannot import from either. AGENTS.md SS9's
 * manual-mirror rule governs all three - CHANGE THEM IN ONE COMMIT.
 *
 * The verification record below was produced once, against the live chain, and
 * is reproduced verbatim rather than summarised. It is the evidence for a WRITE
 * path where a wrong selector costs a user real BNB in a reverted transaction,
 * and a summary of evidence is not evidence.
 */
